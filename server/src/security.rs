...
use crate::{
    ...
    classrooms, classroom_members, players, player_secrets,
};
use crate::auth::teacher_secrets;
use hmac::{Hmac, Mac};
use sha2::Sha256;
...
/// Restore an account session via email magic link.
/// Authenticates via HMAC signature generated by the Vercel API.
#[reducer]
pub fn restore_account_via_email(ctx: &ReducerContext, email: String, code: String, signature: String, expires_at_ms: u64) -> Result<(), String> {
    ...
    if expected_sig != signature {
        return Err("Invalid or forged verification pin.".into());
    }

    // Lookup player_secrets by matching email explicitly
    let target_email = email.trim().to_lowercase();
    let secret = ctx.db.player_secrets().iter().find(|s| s.email.as_deref().map(|e| e.to_lowercase()) == Some(target_email.clone()))
        .ok_or("Email not verified or no account associated.")?;

    // Find the RecoveryKey for this identity to get the token
    let record = ctx.db.recovery_keys().iter().find(|k| k.owner == secret.identity)
        .ok_or("No recovery token generated for this account")?;
