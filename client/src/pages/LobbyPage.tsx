...
export default function LobbyPage({ myPlayer, myIdentityHex, onStartSprint, onEnterClassroom }: Props) {
  ...
  const tier1Unlocked = myIdentityHex
    ? (unlockLogs as any[]).some((u: any) => u.playerIdentity.toHexString() === myIdentityHex && u.tier === 1)
    : false;
        ...
        <>
          {tier1Unlocked && (
            <div className="card" style={{
              ...
              <div>
                <div style={{ fontWeight: 700, color: 'var(--accent)' }}>{t('unlock.tier1Title' as any)}</div>
                <div style={{ fontSize: 13, color: 'var(--muted)' }}>{t('unlock.tier1Desc' as any)}</div>
              </div>
            </div>
          )}
          <div className="card">
            ...
            <MasteryGrid answers={myAnswers as any[]} problemStats={problemStats as any[]} tier1Unlocked={tier1Unlocked} />
          </div>
        </>
      )}
    </div>
