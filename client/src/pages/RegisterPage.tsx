...
import { generateGamertag } from '../utils/nameGen.js';
import SplashGrid from '../components/SplashGrid.js';
import LanguagePicker from '../components/LanguagePicker.js';
import PageContainer from '../components/PageContainer.js';
import { CodeIcon, EmailColorIcon } from '../components/Icons.js';
...
export default function RegisterPage({ onRegistered }: Props) {
  ...
  const [username, setUsername] = useState(() => generateGamertag());
  ...
  const handleSubmit = async (e: FormEvent) => {
    ...
    try {
      // Always register as Solo base identity. 
      // If they came via a ?join=123 link, it's saved in localStorage and App.tsx will auto-join them once in the Lobby.
      await Promise.race([
        ...
        <div className="flex flex-col items-start sm:items-center">
          ...
          {!showRestore && (
            <div className="mt-1 flex flex-col items-start gap-1 sm:mt-6 sm:items-center sm:gap-2">
              <p className="max-w-[200px] text-xs font-bold leading-tight text-slate-500 dark:text-slate-400 sm:max-w-md sm:text-lg sm:text-slate-600 dark:sm:text-slate-300">
                {t('register.tagline')}
              </p>
              <div className="h-0.5 w-8 rounded-full bg-brand-yellow sm:h-1 sm:w-12 mt-2" />
            </div>
          )}
        </div>
      </div>
      ...
      <div className="relative w-full max-w-md animate-in fade-in slide-in-from-bottom-12 duration-1000 delay-300">
        {!showRestore ? (
          <div className="rounded-[28px] border border-slate-200 bg-white p-8 shadow-sm dark:border-slate-800 dark:bg-slate-800/80">
            ...
            <h2 className="mb-8 flex items-center justify-center gap-3 text-2xl font-black tracking-tight text-slate-900 dark:text-white">
            ...
            <form onSubmit={handleSubmit} className="flex flex-col gap-5">
                ...
                {error && (
                  <p className="flex items-center justify-center gap-2 font-medium text-red-500 text-sm">
            ...
            <div className="mt-8 flex items-center gap-4 text-slate-300 dark:text-slate-700">
              ...
              <span className="text-[10px] font-bold uppercase tracking-widest">{t('common.or')}</span>
              <div className="h-px flex-1 bg-current" />
            </div>
            ...
            {restoreMode === 'select' ? (
              <>
                ...
                <div className="flex flex-col gap-3">
                  ...
                  >
                    <CodeIcon className="h-12 w-12 shrink-0 transition-transform group-hover:scale-105" />
                    <div className="flex flex-col min-w-0">
                      <span className="text-[15px] font-black tracking-wide text-slate-800 dark:text-slate-100">{t('register.iknowCode', { defaultValue: 'I have a 12-digit code' })}</span>
                      <span className="text-[12px] font-medium text-slate-500 dark:text-slate-400 truncate mt-0.5 border-none">{t('register.codeDesc', { defaultValue: 'Printed on your recovery card' })}</span>
                    </div>
                  </button>

                  <button
                    onClick={() => { setRestoreMode('email'); setCode(''); }}
                    className="group flex w-full items-center gap-4 rounded-[24px] border-2 border-slate-100 bg-white p-3.5 shadow-sm transition-all hover:-translate-y-0.5 hover:border-slate-200 hover:shadow-md active:scale-[0.98] dark:border-slate-700/50 dark:bg-slate-800 dark:hover:bg-slate-700/80 dark:hover:border-slate-600 text-left"
                  >
                    <EmailColorIcon className="h-12 w-12 shrink-0 transition-transform group-hover:scale-105" />
                    <div className="flex flex-col min-w-0">
                      <span className="text-[15px] font-black tracking-wide text-slate-800 dark:text-slate-100">{t('register.iknowEmail', { defaultValue: 'I know my email' })}</span>
                      <span className="text-[12px] font-medium text-slate-500 dark:text-slate-400 truncate mt-0.5 border-none">{t('register.emailDesc', { defaultValue: 'Login via magic link' })}</span>
                    </div>
                  </button>
                </div>
              </>
              ...
              <form onSubmit={handleEmailRestore} className="flex flex-col gap-6">
                ...
                <button type="submit" disabled={restoring || !restoreEmail} className="group relative h-16 w-full cursor-pointer overflow-hidden rounded-[20px] bg-brand-yellow px-8 py-4 transition-all hover:scale-[1.02] active:scale-95 disabled:cursor-not-allowed disabled:opacity-50 flex items-center justify-center uppercase font-black text-slate-900">
              ...
              <form onSubmit={handleVerifyEmailRestore} className="flex flex-col gap-6">
                ...
                <button type="submit" disabled={restoring || syncing || code.length !== 6} className="group relative h-16 w-full cursor-pointer overflow-hidden rounded-[20px] bg-[#10B981] px-8 py-4 transition-all hover:scale-[1.02] active:scale-95 disabled:cursor-not-allowed disabled:opacity-50 flex items-center justify-center uppercase font-black tracking-wider text-white">
                  ...
                  <>
                    ...
                    >
                      {restoring ? t('register.restoring') : (
                         <>{t('register.restore')} <span className="transition-transform ltr:group-hover:translate-x-1 rtl:group-hover:-translate-x-1 rtl:group-hover:rotate-12 inline-block ml-1">↺</span></>
                      )}
                    </button>
                  </>
                )}
